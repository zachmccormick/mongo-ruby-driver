    ...
    module Selectable
      ...
      def select_server(cluster)
        ...
        while (deadline - Time.now) > 0
          ...
          ::Rails.logger.info("[jontest] could not get server for cluster #{cluster.inspect}: servers #{servers.inspect}, round trip time is #{cluster.servers.map(&:average_round_trip_time)}")
          cluster.scan!
        end
